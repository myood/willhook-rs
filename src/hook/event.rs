...
impl From<WPARAM> for KeyPress {
    fn from(code: WPARAM) -> Self {
        use KeyPress::*;
        match code as u32 {
            WM_KEYDOWN => Down(false),
            WM_KEYUP => Up(false),
            WM_SYSKEYDOWN => Down(true),
            WM_SYSKEYUP => Up(true),
            _ => Other(code),
        }
    }
...
impl From<WPARAM> for MouseClick {
    fn from(value: WPARAM) -> Self {
        use MouseClick::*;
        match value as u32 {
            WM_LBUTTONDOWN | WM_LBUTTONUP | WM_RBUTTONDOWN | WM_RBUTTONUP | WM_MBUTTONDOWN | WM_MBUTTONUP | WM_XBUTTONDOWN | WM_XBUTTONUP => SingleClick,
